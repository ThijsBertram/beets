...
class RekordboxSyncPlugin(BeetsPlugin):
    def __init__(self):
        ...
        self.xml_path = os.path.abspath(os.path.expanduser(self.config['rekordbox_xml_path'].as_str()))
        self.backup_dir = os.path.abspath(os.path.expanduser(self.config['backup_directory'].as_str()))
        ...
        self.sync_command.parser.add_option('--fields', dest='fields', help="Fields to syn")
        self.sync_command.parser.add_option('--rkbx-xml', dest='rkbx_xml', default=self.xml_path, help="Fields to sync")

        self.sync_command.func = self.sync_metadata
    ...
    def _parse_rekordbox_tracks(self, root):
        ...
        for track_elem in root.findall('.//TRACK'):
            ...
            if not track_data['TrackID'] or not track_data['Name'] or not track_data['Location']:
                self.logger.warning(f"Skipping invalid track with missing fields: {track_data}")
                continue
    ...
    def _resolve_conflict(self, beets_value, rb_value, field_name, rb_track, item):
        """Resolve metadata conflicts based on the configuration."""

        conflict_rule = self.config['conflict_resolution'].get()[field_name]
        ...
        if conflict_rule == 'beets':
            ...
            return rb_value if rb_track.get('LastModified', 0) > item.added else beets_value
        return beets_value
    ...
    def sync_metadata(self, lib, opts, args):
        ...
        # DATA FOR REKORDBOX AND BEETS 
        rb_tracks = self._parse_rekordbox_tracks(root)
        items = (i for i in lib.items())  # Use a generator for large collections
        updated_beets = 0
        updated_rekordbox = 0
        not_found = 0

        # Get field mappings
        mapping_beets_to_rekordbox = self.config['field_mapping']['beets_to_rekordbox'].get()
        mapping_rekordbox_to_beets = self.config['field_mapping']['rekordbox_to_beets'].get()

        # Filter fields if specified
        if fields:
            mapping_beets_to_rekordbox = {k: v for k, v in mapping_beets_to_rekordbox.items() if k in fields}
            mapping_rekordbox_to_beets = {k: v for k, v in mapping_rekordbox_to_beets.items() if v in fields}
        ...
        for item in items:
            if not item.path:
                self.logger.debug(f"Skipping item without a valid path: {item}")
                continue

            # PROCESS PATH STUFF
            os_path, os_filename, rekordbox_file, rekordbox_path = self._process_path(item)

            # print(os_path)
            # print(os_filename)
            # print(rekordbox_file)
            ...
            if not rb_track:
                self.logger.warning(f"Track not found in Rekordbox: {rekordbox_file} ({item.title} - {item.artist})")
                not_found += 1
                continue
            ...
            updated_beets += self._update_beets_metadata(item, rb_track, mapping_rekordbox_to_beets)
            updated_rekordbox += self._update_rekordbox_metadata(root, rb_track, item, mapping_beets_to_rekordbox)



            # Synchronize tags
            if 'tags' in mapping_beets_to_rekordbox or 'tags' in mapping_rekordbox_to_beets.values():
        ...
        self.logger.info(f"Skipped {not_found} tracks not found in Rekordbox.")
...
#                 rekordbox_path = short_path.replace(' ', '%20')
    ...
    def _update_beets_metadata(self, item, rb_track, mapping):
        """Update Beets metadata for an item."""
        updated = 0
    ...
    def _sync_tags(self, item, rb_track, reverse=False):
        ...
        # if reverse:  # Rekordbox to Beets
        #     print()
        #     print('rkbx to beets')
        #     rb_tags_raw = rb_track.get('Comments', '')
        #     print(rb_tags_raw)
        #     rb_tags = parse_tags(rb_tags_raw, delimiter=' / ', prefix='*/', suffix='*/')
        #     beets_tags = parse_tags(item.tags, delimiter=',')

        #     combined_tags = sorted(set(tag.lower() for tag in beets_tags + rb_tags))
        ...
        #     combined_tags = sorted(set(tag.lower() for tag in beets_tags + rb_tags))
        #     new_tags = f"*/ {' / '.join(combined_tags)} */"

        #     print(new_tags)

        #     if new_tags != rb_tags_raw:
        #         rb_track['Comments'] = new_tags
